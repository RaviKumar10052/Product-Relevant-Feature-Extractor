...
def main():
    threshold = 1        #Raghav change 5000 to 1
    ...
    for i in noun_postags:
        if snowball.stem(i) in tf:  # Raghav change 'if tf.has_key(snowball.stem(i)):' to 'snowball.stem(i) in tf'
    ...
    for token, count in tf.items():   #Raghav change iteritems to items
